...
fn main() {
    ...
    println!("Render time: {} s", t0.elapsed().as_secs_f32());
    write_image(&args[1], &pixels, bounds).expect("error writing PNG file");
}

/// Render a rectangle of the Mandelbrot set into a buffer of pixels.
///
/// The `bounds` argument gives width and height of the buffer `pixels`
/// which holds one grayscale pixel per byte. The `upper_left` and `lower_rigth`
...
/// Try to determin is `c` is in the Mandelbrot set, using at most `limit`
...
/// The `uppe_left and `lower_right` parameters are points on the complex
...
fn test_pixel_to_point() {
    assert_eq!(
        pixel_to_point((100, 100), (25, 75), Complex::new(-1.0, 1.0), Complex::new(1.0, -1.0)),
        Complex::new(-0.5, -0.5)
    );
}
